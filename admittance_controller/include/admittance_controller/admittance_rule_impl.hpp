  ...
  const rclcpp::Duration & period, trajectory_msgs::msg::JointTrajectoryPoint & desired_joint_state)
{
  ...
  for (size_t i = 0; i < num_joints_; ++i)
  {
    ...
    desired_joint_state.positions[i] =
      current_joint_state.positions[i] + admittance_state_.joint_pos[idx];
    desired_joint_state.velocities[i] =
      current_joint_state.velocities[i] + admittance_state_.joint_vel[idx];
    desired_joint_state.accelerations[i] =
      current_joint_state.accelerations[i] + admittance_state_.joint_acc[idx];
  }
